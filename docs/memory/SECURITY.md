...
- requires every persistent JSON-publication directory to report the same filesystem device, then actively creates and identity-verifies a hard-link probe only in non-data `locks/` after the empty layout exists but before a lock, manifest, journal, or record can be published;
