...
The hard-link probe captures the source inode identity from its opened handle, requires the destination to share that identity and have a real multi-link count, and cleans only exact random paths that still have a proven probe identity. Expected unsupported-operation codes are mapped to the path-free `MEMORY_HARD_LINK_UNAVAILABLE` error; unrelated I/O and identity errors remain generic filesystem failures. An ordinary unsupported result followed by successful identity-proven cleanup removes both probe names; on a new sidecar it leaves only empty version-1 layout directories, and an existing sidecar's stored files remain unchanged. Separately, if identity acquisition or revalidation, an I/O failure, a semantic mismatch, or a concurrent replacement makes ownership of a probe source or destination pathname uncertain, Safire fails closed and preserves each bounded random pathname whose ownership is uncertain rather than risk deleting another file. Usually one exact name is retained; a persistent parent-revalidation failure after link creation can retain both. Every retry uses fresh random names and neither adopts nor automatically removes retained pathnames, so repeated ownership-uncertain failures can leave up to two additional probe pathnames per failed attempt. A retained `.hard-link-capability-*` pathname is not proof of ownership; stop all memory writers and obtain specialist review before manual removal. There is no copy, rename, native-module, or reduced-safety fallback; the operator must select a supported local filesystem.
