...
Each enabled store instance initializes lazily once, when its first direct store API or memory-tool operation runs. Initialization ensures that the layout directories exist—empty for a new sidecar, potentially populated with retained state for an existing sidecar—and, before publishing a new lock or any JSON, verifies that the root, state, journals, locks, and five immutable collection directories report the same filesystem device, then performs a real hard-link capability probe only in `locks/`. Keeping transient names out of strictly enumerated data directories prevents one process's initialization from disrupting another process's record or recovery scan; later lock-gate and journal directories are created beneath their already-verified parents. The source identity is captured from its opened handle; source and destination must be plain files with the same device/inode identity and link count of at least two. Cleanup checks the captured identity and removes only the exact random probe names it can still prove. A link published immediately before a reported failure is therefore safely recognized and removed. When a recognized unsupported operation is followed by successful identity-proven cleanup, both probe names are removed. If opened-source identity capture, post-link parent revalidation, source or destination `lstat`, a semantic mismatch, or a concurrent replacement prevents ownership from being proved, Safire preserves every ownership-uncertain random pathname from that failed attempt, returns a generic filesystem or path-identity failure, and publishes no new memory state. One pathname is retained in the ordinary uncertain-identity case; a persistent parent-revalidation failure after link creation can retain both exact names. Retries use fresh random names and neither adopt nor automatically remove retained pathnames, so repeated rare failures may leave up to two per failed attempt. A retained probe requires stopped memory writers and specialist review before manual removal. Recognized unsupported-operation results fail as `MEMORY_HARD_LINK_UNAVAILABLE`; unrelated I/O and identity failures retain a generic filesystem failure instead of being misdiagnosed as missing capability. There is no copy, rename, native-module, or weaker-locking fallback. The ordinary, fully cleaned unsupported case publishes no new lock, manifest, journal, transaction guard, or record, and retrying the probe against an existing sidecar never rewrites existing JSON.
