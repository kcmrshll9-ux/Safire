...
const store = createMemoryStore({
  ...
  profile,
  faultInjector(stage) {
    if (stage !== 'after_journal_create') return undefined;
    process.stdout.write('JOURNALED\n');
    return new Promise(() => {});
  },
});
...
await store.recordEvents([{
  ...
  speech_act: 'proposal',
  content: 'Recover this journal after the writer is terminated.',
  occurred_at: '2026-08-14T17:00:00.000Z',
  context: { conversation_id: 'hard-kill.recovery', turn_id: 'turn.1' },
  source: { stream: 'hard-kill.recovery', event_id: 'turn.1' },
}]);
