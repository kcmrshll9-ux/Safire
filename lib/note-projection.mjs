...
function evidenceInfoKind(info = '') {
  ...
  return /safire[^a-z0-9\r\n]{0,16}evidence/i.test(normalized) ? 'sensitive' : 'ordinary';
}
...
function parseEvidenceBody(lines = []) {
  ...
  return { valid: true, fields, publicLineIndexes };
}
...
export function parsePublicTasks(content = '', notePath = '') {
  const records = lineRecords(content);
...
export function parseEvidenceReceipts(content = '', notePath = '', options = {}) {
  ...
  for (const block of blocks) {
    ...
    const receipt = receiptFromFields(parsed.fields, notePath, receiptIndex, includePrivate, now);
    if (receipt.claim || parsed.fields.get('id')) receipts.push(receipt);
    receiptIndex += 1;
  }
  return receipts;
}
...
export function semanticMarkdownContent(content = '') {
  const projected = publicEvidenceContent(content);
