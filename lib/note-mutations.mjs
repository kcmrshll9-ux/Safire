...
import { assertNoteRevision, noteRevision } from './note-revision.mjs';
...
export function createNoteMutator({
  ...
  async function mutateLocked(absolutePath, transform, { requireExisting = true, expectedRevision } = {}) {
    ...
    assertNoteRevision(snapshot?.content, expectedRevision);
    ...
    const backup = snapshot && backupWrites ? await publishBackup(absolutePath, snapshot) : null;
    ...
    return { backup, value: next.value, revision: noteRevision(next.content) };
  }
  ...
  return Object.freeze({
    ...
    async replace(absolutePath, content, options = {}) {
      noteRelativePath(absolutePath);
      return runMutation([absolutePath], () => mutateLocked(absolutePath, () => content, { ...options, requireExisting: true }));
    },
    ...
    async renameWithLinks(sourcePath, destinationPath, rewrites) {
      noteRelativePath(sourcePath);
      noteRelativePath(destinationPath);
      const edits = rewrites.map(edit => ({ ...edit, absolute: path.join(vaultDir, edit.path) }));
      for (const edit of edits) noteRelativePath(edit.absolute);
      return runMutation([sourcePath, destinationPath, ...edits.map(e=>e.absolute)], async () => {
        if (pathKey(sourcePath) === pathKey(destinationPath)) throw new Error('Choose a different note path');
        await assertContainedPath(vaultDir, destinationPath, { allowMissing: true, fsApi });
        try { await fsApi.lstat(destinationPath); throw new Error('Destination already exists'); }
        catch (error) { if (error.code !== 'ENOENT') throw error; }
        const snapshots = new Map();
        for (const edit of edits) {
          const snapshot = await readStable(fsApi, vaultDir, edit.absolute);
          assertNoteRevision(snapshot.content, edit.revision);
          snapshots.set(edit.absolute, snapshot);
        }
        const sourceEdit = edits.find(edit=>pathKey(edit.absolute) === pathKey(sourcePath));
        if (!sourceEdit) throw new Error('Missing source note in rename plan');
        const backups = [];
        for (const edit of edits) backups.push(await publishBackup(edit.absolute, snapshots.get(edit.absolute)));
        await ensureDirectory(fsApi, vaultDir, path.dirname(destinationPath));
        const destinationIdentity = await writeExclusive(fsApi, destinationPath, sourceEdit.content);
        const applied = [];
        try {
          for (const edit of edits) {
            if (edit === sourceEdit || edit.content === String(snapshots.get(edit.absolute).content)) continue;
            await publishReplacement(edit.absolute, edit.content, snapshots.get(edit.absolute).stat);
            applied.push(edit);
          }
          await assertContainedPath(vaultDir, sourcePath, { fsApi });
          const destination = await readStable(fsApi, vaultDir, destinationPath);
          if (!sameIdentity(destination.stat, destinationIdentity)) throw changedError();
          assertNoteRevision(destination.content, noteRevision(sourceEdit.content));
          const current = await fsApi.lstat(sourcePath, { bigint: true });
          if (!sameSnapshot(current, snapshots.get(sourceEdit.absolute).stat)) throw changedError();
          await fsApi.unlink(sourcePath);
        } catch (error) {
          for (const edit of applied.reverse()) {
            try {
              const current = await readStable(fsApi, vaultDir, edit.absolute);
              assertNoteRevision(current.content, noteRevision(edit.content));
              await publishReplacement(edit.absolute, snapshots.get(edit.absolute).content, current.stat);
            } catch { /* Immutable backups remain available for recovery. */ }
          }
          try {
            const destination = await readStable(fsApi, vaultDir, destinationPath);
            assertNoteRevision(destination.content, noteRevision(sourceEdit.content));
            await unlinkIfOwned(fsApi, destinationPath, destinationIdentity);
          } catch { /* Preserve an independently edited destination. */ }
          throw error;
        }
        return { backups, updatedLinks: edits.reduce((sum,e)=>sum+e.count,0), updatedNotes: applied.length };
      });
    },
...
export async function readContainedFile(vaultDir, absolutePath, encoding, { fsApi = fs, maxBytes } = {}) {
  const snapshot = await readStable(fsApi, vaultDir, absolutePath, { maxBytes });
