...
export class MemoryHardLinkUnavailableError extends MemoryFilesystemError {
  constructor() {
    super('Safire memory requires same-directory hard-link support', {
      code: 'MEMORY_HARD_LINK_UNAVAILABLE',
    });
    this.name = 'MemoryHardLinkUnavailableError';
  }
}
...
function pathIdentity(stat) {
  return Object.freeze({
    dev: String(stat.dev),
    ino: String(stat.ino),
    birthtimeNs: stat.birthtimeNs == null ? null : String(stat.birthtimeNs),
  });
}
...
export async function ensureMemoryLayout(vaultPath) {
  ...
  // Exclusive JSON publication and lock-owner metadata require a real hard
  // link. Prove that every persistent publication directory is on the same
  // filesystem, then probe in locks/, which is never scanned as a strict JSON
  // collection. Probing data directories before acquiring the vault lock would
  // expose transient noncanonical names to another process's strict scan.
  ...
  const publicationDirectoryStates = await Promise.all(
    publicationDirectories.map(directory => knownOrCurrentDirectory(directory)),
  );
  const locksDevice = publicationDirectoryStates[publicationDirectories.indexOf(locksDir)].identity.dev;
  if (publicationDirectoryStates.some(state => state.identity.dev !== locksDevice)) {
    throw new MemoryHardLinkUnavailableError();
  }
  await assertHardLinkPublicationSupported(locksDir);
...
function hardLinkPublicationError(error) {
  if (error instanceof MemoryHardLinkUnavailableError) return error;
  if (HARD_LINK_UNAVAILABLE_CODES.has(error?.code)) {
    return new MemoryHardLinkUnavailableError();
  }
  return error;
}

function hardLinkProbeOperationError(error) {
  if (error instanceof MemoryFilesystemError) return error;
  return new MemoryFilesystemError('Safire memory hard-link capability probe failed', {
    code: 'MEMORY_FILESYSTEM_ERROR',
  });
}

function hardLinkProbeIdentityError() {
  return new MemoryFilesystemError('Safire memory hard-link capability probe identity changed', {
    code: 'MEMORY_PATH_IDENTITY',
  });
}

async function plainFileIdentity(target) {
  try {
    const stat = await fs.lstat(target, { bigint: true });
...
async function unlinkProbeIfOwned(target, expectedIdentity, parentIdentity) {
  if (!expectedIdentity) return false;
  await revalidateDirectory(parentIdentity);
  const currentIdentity = await plainFileIdentity(target);
  if (!sameIdentity(expectedIdentity, currentIdentity)) {
    throw hardLinkProbeIdentityError();
  }
  return unlinkWithStableParent(target, parentIdentity);
}

export async function assertHardLinkPublicationSupported(directory) {
  const {
    realDirectory, directoryIdentity,
  } = await safeExistingDirectory(directory, directory);
  const token = randomOpaqueToken(24);
  const source = resolveContainedPath(realDirectory, `.hard-link-capability-${token}.source`);
  const destination = resolveContainedPath(realDirectory, `.hard-link-capability-${token}.link`);
  ...
  try {
    ...
    sourceCreated = true;
    let openedSourceStat;
    try {
      openedSourceStat = await sourceHandle.stat({ bigint: true });
    } catch (initialStatError) {
      // The exclusive open proves creation but not a pathname identity. Make
      // one recovery attempt on that same still-open handle so cleanup can be
      // ownership-fenced, while retaining the original operation failure.
      try {
        const recoveredSourceStat = await sourceHandle.stat({ bigint: true });
        if (recoveredSourceStat.isFile() && !recoveredSourceStat.isSymbolicLink()) {
          sourceIdentity = pathIdentity(recoveredSourceStat);
        }
      } catch {
        // Without a handle-proven identity, preserve the pathname fail closed.
      }
      throw initialStatError;
    }
    if (!openedSourceStat.isFile() || openedSourceStat.isSymbolicLink()) {
      throw hardLinkProbeIdentityError();
    }
    sourceIdentity = pathIdentity(openedSourceStat);
    await sourceHandle.writeFile('safire-memory-hard-link-capability-v1\n', 'utf8');
    ...
    try {
      ...
      // A resolved hard-link operation proves that the destination named the
      // captured source inode at publication time. Later cleanup must still
      // re-stat and match this identity before unlinking either pathname.
      destinationIdentity = sourceIdentity;
    } catch (error) {
      // A platform wrapper can report failure after the link was published.
      // Only claim that pathname for cleanup if it still names our source
      // inode; a collision or replacement is deliberately preserved.
      const possibleDestinationIdentity = await plainFileIdentity(destination);
      if (sameIdentity(sourceIdentity, possibleDestinationIdentity)) {
        destinationIdentity = possibleDestinationIdentity;
      } else if (possibleDestinationIdentity) {
        throw hardLinkProbeIdentityError();
      }
      throw hardLinkPublicationError(error);
    }
    await revalidateDirectory(directoryIdentity);
    ...
    const currentSourceIdentity = pathIdentity(sourceStat);
    const currentDestinationIdentity = pathIdentity(destinationStat);
    if (sameIdentity(sourceIdentity, currentDestinationIdentity)) {
    ...
    if (!sourceStat.isFile()
        ...
        || !sameIdentity(sourceIdentity, currentSourceIdentity)) {
      throw hardLinkProbeIdentityError();
    }
    if (!sameIdentity(sourceIdentity, currentDestinationIdentity)
        || sourceStat.nlink < 2n
    ...
    operationError = hardLinkProbeOperationError(error);
  } finally {
    await sourceHandle?.close().catch(() => {});
  }

  let cleanupError = null;
  for (const [target, expectedIdentity] of [
    [destination, destinationIdentity],
    [source, sourceCreated ? sourceIdentity : null],
  ]) {
    if (!expectedIdentity) continue;
    try {
      await unlinkProbeIfOwned(target, expectedIdentity, directoryIdentity);
    } catch (error) {
      cleanupError ||= error;
    }
  }
  try {
    await syncDirectory(realDirectory, directoryIdentity);
  } catch (error) {
    cleanupError ||= error;
  }
  if (cleanupError) throw hardLinkProbeOperationError(cleanupError);
