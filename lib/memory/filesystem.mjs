...
export async function readJsonWithDigest(rootDir, targetPath) {
  ...
  let bytes;
  try {
    const openedIdentity = pathIdentity(await handle.stat({ bigint: true }));
...
async function replaceJsonAtomic(rootDir, targetPath, value, { beforePublish = null } = {}) {
...
async function readLockSnapshot(layout) {
  let handle;
  try {
    const {
      target, parentIdentity, fileIdentity,
    } = await safeFileTarget(layout.locksDir, layout.lockPath, { mustExist: true });
    await revalidateDirectory(parentIdentity);
    handle = await fs.open(target, fsConstants.O_RDONLY);
    const stat = await handle.stat({ bigint: true });
    if (!sameIdentity(fileIdentity, pathIdentity(stat))) throw directoryChangedError();
    const bytes = await handle.readFile();
    await revalidateDirectory(parentIdentity);
    let metadata = null;
    try { metadata = JSON.parse(bytes.toString('utf8')); } catch { /* Invalid locks remain held for operator review. */ }
...
async function createVaultLock(layout) {
  ...
  // Publish only after the complete metadata file has been flushed. The same-directory
...
async function lockStillOwned(layout, token) {
  const snapshot = await readLockSnapshot(layout);
    ...
    try {
      ...
      const lock = {
        ...
        async release() {
          ...
          const snapshot = await readLockSnapshot(layout);
          if (!snapshot?.metadata?.token || snapshot.metadata.token !== token) {
            released = true;
            throw new VaultLockOwnershipError();
          }
          const { parentIdentity } = await safeFileTarget(
            layout.locksDir,
            layout.lockPath,
            { mustExist: true },
          );
          const current = await readLockSnapshot(layout);
          if (!current || current.digest !== snapshot.digest || current.metadata?.token !== token) {
            released = true;
            throw new VaultLockOwnershipError();
          }
          await unlinkWithStableParent(layout.lockPath, parentIdentity);
          await syncDirectory(layout.locksDir, parentIdentity);
          released = true;
          return true;
        },
      };
