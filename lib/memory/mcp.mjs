...
export const MEMORY_MCP_SERVER_VERSION = '1.0.0';
...
export const memoryMcpToolSchemas = Object.freeze({
  ...
  memory_search: z.object({
    query: z.string().max(2_000).optional().describe('Optional visible text to search for.'),
...
export function registerMemoryMcpTools(server, store) {
  ...
  registerJsonTool(
    ...
    (memoryStore, { id }) => memoryStore.get(id),
  );
  registerJsonTool(
    server,
    store,
  ...
  registerJsonTool(
    ...
    (memoryStore, { ids }) => memoryStore.recall(ids),
  );
  registerJsonTool(
    server,
    store,
...
export function createMemoryMcpServer({
  ...
  const memoryStore = store || createMemoryStore({ vaultDir, profile, enabled });
