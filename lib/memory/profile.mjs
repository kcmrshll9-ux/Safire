...
function assertKnownKeys(value, allowed, label) {
  for (const key of Object.keys(value)) {
    if (!allowed.has(key)) validationError(`${label} contains an unsupported field`);
  }
}
...
function normalizeStableId(value, label) {
  ...
  if (value.length > SCHEMA_LIMITS.opaqueIdLength) {
...
function normalizeDisplayName(value) {
  ...
  if (value.length > 200) validationError('actor display_name is invalid');
