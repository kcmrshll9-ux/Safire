...
const RAW_JWT_CANDIDATE_PATTERN = new RegExp(
  `(?<![A-Za-z0-9])([A-Za-z0-9][A-Za-z0-9_-]{0,${RAW_JWT_LIMITS.headerSegmentLength - 1}})`
...
function containsCredibleRawJwt(candidate) {
  for (const match of candidate.matchAll(RAW_JWT_CANDIDATE_PATTERN)) {
    const start = match.index;
    const end = start + match[0].length;
    if (candidate[start - 1] === '.' && BASE64URL_CHARACTER_PATTERN.test(candidate[start - 2] || '')) continue;
    if (candidate[end] === '.' && BASE64URL_CHARACTER_PATTERN.test(candidate[end + 1] || '')) continue;
    const tokenLength = match[1].length + match[2].length + match[3].length + 2;
    if (tokenLength > RAW_JWT_LIMITS.totalLength) continue;
    const header = parseJwtJsonObject(match[1]);
    const payload = parseJwtJsonObject(match[2]);
    const signatureCandidates = [match[3]];
    let delimiterCandidate = match[3];
    for (let delimiterLength = 1; delimiterLength <= 3; delimiterLength += 1) {
      if (!/[_-]$/.test(delimiterCandidate)) break;
      delimiterCandidate = delimiterCandidate.slice(0, -1);
      signatureCandidates.push(delimiterCandidate);
    }
    const hasCredibleSignature = signatureCandidates.some(segment =>
      decodeCanonicalBase64url(segment)?.length >= RAW_JWT_LIMITS.minimumSignatureBytes
    );
    if (header
        && payload
