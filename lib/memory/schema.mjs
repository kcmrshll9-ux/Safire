...
const SENSITIVE_TEXT_PATTERNS = Object.freeze([
  ...
  /(?<![A-Za-z0-9])github_pat_[A-Za-z0-9_]{82}(?![A-Za-z0-9])/,
  /(?<![A-Za-z0-9])(?:gh[pousr]_[A-Za-z0-9]{20,}|xox[baprs]-[A-Za-z0-9-]{10,}|sk-[A-Za-z0-9_-]{20,})(?![A-Za-z0-9])/i,
  /(?<![A-Za-z0-9])npm_[A-Za-z0-9]{36}(?![A-Za-z0-9])/,
  /(?<![A-Za-z0-9])glpat-[A-Za-z0-9_-]{27,300}\.[a-z0-9]{9}(?![A-Za-z0-9]|\.[A-Za-z0-9])/,
  /(?<![A-Za-z0-9])glpat-[A-Za-z0-9_-]{20}(?![A-Za-z0-9]|\.[A-Za-z0-9])/,
  /(?<![A-Za-z0-9])AIza[A-Za-z0-9_-]{35}(?![A-Za-z0-9])/,
  /(?<![A-Za-z0-9])(?:sk|rk)_(?:test|live)_[A-Za-z0-9]{20,247}(?![A-Za-z0-9])/,
  /(?<![A-Za-z0-9])hf_[A-Za-z0-9]{34}(?![A-Za-z0-9])/,
...
const RAW_JWT_LIMITS = Object.freeze({
  headerSegmentLength: 1_024,
  payloadSegmentLength: 16_384,
  signatureSegmentLength: 8_192,
  minimumSignatureBytes: 16,
});
const RAW_JWT_SCAN_BUDGET = Object.freeze({
  bodyCandidateChecks: 128,
  headerCandidateChecks: 128,
});
const RAW_JWT_BODY_PATTERN = new RegExp(
  // The zero-width match lets each dot begin an overlapping three-part candidate.
  `(?=\\.([A-Za-z0-9_-]{1,${RAW_JWT_LIMITS.payloadSegmentLength}})`
    + `\\.([A-Za-z0-9_-]{1,${RAW_JWT_LIMITS.signatureSegmentLength}}))`,
  'g',
);
const FATAL_UTF8_DECODER = new TextDecoder('utf-8', { fatal: true });
const ASCII_ALPHANUMERIC_CHARACTER_PATTERN = /^[A-Za-z0-9]$/;
...
function parseJwtJsonObject(segment) {
  const decoded = decodeCanonicalBase64url(segment);
  if (!decoded) return null;
  try {
    const value = JSON.parse(FATAL_UTF8_DECODER.decode(decoded));
...
function isCredibleJwtHeader(segment) {
  const header = parseJwtJsonObject(segment);
  return Boolean(
    header
...
function base64urlSextetValue(character) {
  const code = character.codePointAt(0);
  if (code >= 0x41 && code <= 0x5a) return code - 0x41;
  if (code >= 0x61 && code <= 0x7a) return code - 0x61 + 26;
  if (code >= 0x30 && code <= 0x39) return code - 0x30 + 52;
  if (character === '-') return 62;
  return character === '_' ? 63 : -1;
}

function isCredibleJwtSignaturePrefix(signature, end) {
  if (end < 1) return false;
  const remainder = end % 4;
  if (remainder === 1) return false;
  const finalSextet = base64urlSextetValue(signature[end - 1]);
  if (finalSextet < 0) return false;
  if (remainder === 2 && (finalSextet & 0x0f) !== 0) return false;
  if (remainder === 3 && (finalSextet & 0x03) !== 0) return false;
  return Math.floor(end * 6 / 8) >= RAW_JWT_LIMITS.minimumSignatureBytes;
}

function hasAdjacentBase64urlSegmentBefore(candidate, start) {
  return candidate[start - 1] === '.'
    && BASE64URL_CHARACTER_PATTERN.test(candidate[start - 2] || '');
}

function hasAdjacentBase64urlSegmentAfter(candidate, end) {
  return candidate[end] === '.'
    && BASE64URL_CHARACTER_PATTERN.test(candidate[end + 1] || '');
}

function containsCredibleJwtHeaderBefore(candidate, firstDot, scanBudget) {
  const minimumStart = Math.max(0, firstDot - RAW_JWT_LIMITS.headerSegmentLength);
  let earliestStart = firstDot;
  while (earliestStart > minimumStart
      && BASE64URL_CHARACTER_PATTERN.test(candidate[earliestStart - 1])) {
    earliestStart -= 1;
  }
  for (let start = earliestStart; start < firstDot; start += 1) {
    if (!ASCII_ALPHANUMERIC_CHARACTER_PATTERN.test(candidate[start])) continue;
    if (ASCII_ALPHANUMERIC_CHARACTER_PATTERN.test(candidate[start - 1] || '')) continue;
    if (hasAdjacentBase64urlSegmentBefore(candidate, start)) continue;
    if (scanBudget.headerCandidateChecks === 0) return true;
    scanBudget.headerCandidateChecks -= 1;
    if (isCredibleJwtHeader(candidate.slice(start, firstDot))) return true;
  }
  return false;
}

function containsCredibleJwtSignatureAfter(candidate, secondDot, signatureRun) {
  const signatureStart = secondDot + 1;
  const signatureEnd = signatureStart + signatureRun.length;
  if (candidate[signatureEnd] === '='
      || hasAdjacentBase64urlSegmentAfter(candidate, signatureEnd)) {
    return false;
  }
  if (isCredibleJwtSignaturePrefix(signatureRun, signatureRun.length)) {
    return true;
  }
  // Base64url uses `_` and `-`, but callers also use them as token delimiters.
  for (let end = 0; end < signatureRun.length; end += 1) {
    if (signatureRun[end] !== '_' && signatureRun[end] !== '-') continue;
    if (isCredibleJwtSignaturePrefix(signatureRun, end)) return true;
  }
  return false;
}

function containsCredibleRawJwt(candidate) {
  // `_` and `-` can be either base64url data or external delimiters. Bound
  // semantic retries across the whole value and reject ambiguous excess work.
  const scanBudget = { ...RAW_JWT_SCAN_BUDGET };
  for (const match of candidate.matchAll(RAW_JWT_BODY_PATTERN)) {
    const firstDot = match.index;
    const secondDot = firstDot + match[1].length + 1;
    if (!containsCredibleJwtSignatureAfter(candidate, secondDot, match[2])) continue;
    if (scanBudget.bodyCandidateChecks === 0) return true;
    scanBudget.bodyCandidateChecks -= 1;
    if (!parseJwtJsonObject(match[1])) continue;
    if (containsCredibleJwtHeaderBefore(candidate, firstDot, scanBudget)) return true;
  }
  return false;
}
...
export function containsDisallowedSensitiveMaterial(value) {
  ...
  return SENSITIVE_TEXT_PATTERNS.some(pattern => pattern.test(candidate))
    || containsCredibleRawJwt(candidate);
}
...
export const opaqueIdSchema = z.string()
  ...
  .superRefine((value, context) => {
    if (value.length <= SCHEMA_LIMITS.opaqueIdLength) addSensitiveTextIssue(value, context);
  });
...
const safeAttributeKeySchema = z.string()
  ...
  .refine(key => key.length > 64 || !containsDisallowedSensitiveMaterial(key), 'Sensitive attribute keys are not accepted')
