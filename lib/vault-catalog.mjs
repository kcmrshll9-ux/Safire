...
import { genericIndexContent, publicNoteMetadata, parseEvidenceReceipts, publicEvidenceContent } from './note-projection.mjs';
    ...
    scanLoop: while (directories.length) {
      ...
      for await (const entry of handle) {
        ...
        try {
          ...
          const fingerprint = `projection-2:${stat.dev}:${stat.ino}:${stat.size}:${stat.mtimeMs}:${stat.ctimeMs}:${contentAllowed}`;
