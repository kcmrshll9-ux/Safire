...
export async function createNotesMcpService({ vaultDir: configuredVaultDir }) {
  ...
  return Object.freeze({
    ...
    async readNote(notePath) {
      ...
      return { path: rel, title: titleFromPath(rel), content, revision: noteRevision(content), tags: metadata.tags, links: metadata.links };
    },
    ...
    async updateNote(notePath, content, revision) {
      const { rel, abs } = resolveUserMutationNotePath(notePath);
      const result = await noteMutator.replace(abs, String(content ?? ''), { expectedRevision: revision });
      return { ok: true, path: rel, backup: result.backup, revision: result.revision };
    },
